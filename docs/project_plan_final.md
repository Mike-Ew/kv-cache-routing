...
Large language model serving deployments typically run multiple inference engine instances behind a load balancer. Each instance maintains its own KV cache of previously computed attention states; routing a request to an instance that already holds relevant cached prefixes can eliminate redundant computation and dramatically reduce time-to-first-token (TTFT). \n\nWe propose an **Agentic Routing Gateway** built with **LangGraph** that treats routing not as a static algorithm, but as a stateful decision process. We implement and evaluate six routing policies — round-robin, join-shortest-queue, power-of-two-choices, session-affinity hashing, prefix-key affinity, and a novel load+cache-aware scoring policy — integrated as nodes in a routing graph. We measure ground-truth metrics via vLLM's native Prometheus endpoint, provide the first systematic study of cache directory staleness, and release an open-source implementation compliant with the **Agentic AI** course track.
...
class RoundRobin:
    ...
    def route(self, request, states):
        # Logic to extract prefix, check cache index, check queue depths
...
- **Agentic flow:** Request -> Scrape Metrics -> Update Cache Dir -> Score & Route -> Forward -> Record
- **Policy interface:** `Policy.route(request, states) -> instance_index` called by the routing node
...
- Future work: semantic similarity routing, cross-instance cache migration, adaptive policy switching (agent decides when to switch policies)

### §8 Ethics & Impact (0.5 pages) required by rubric

- **Fairness:** Cache-aware routing can create "hot" instances where lucky users get fast responses and others wait. Discuss tail latency impact.
- **Energy:** Reducing redundant computation saves GPU energy (Green AI).
- **Agentic risks:** Automated decision making (routing) vs deterministic load balancing reliability.
...
| 3–4 | Build router skeleton (LangGraph StateGraph + nodes)                  |
| 5–6 | Build metrics collector node (scrape `/metrics`)                      |
...
├── README.md
├── router/
│   ├── server.py           The system is a **LangGraph-based Gateway** that acts as an intelligent agent in front of vLLM instances:

- **`router/`** — LangGraph application (`server.py`, `graph.py`, `nodes.py`, `state.py`, `policies.py`) that executes the routing logic as a graph of nodes.
- **`workloads/`** — ShareGPT multi-turn replayer, synthetic RAG generator, mixed workload
- **`simulator/`** — Trace-driven scaling simulator with LRU cache model
